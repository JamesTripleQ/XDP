...
public class xdp_driftStats extends BaseShipSystemScript {
    ...
    public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
        ...
        if(ship!=null){
            if(MagicRender.screenCheck(1, ship.getLocation())){
                if(Math.random()>0.9f){
                    ship.addAfterimage(new Color(255,10,0,128), 0, 0, -ship.getVelocity().x, -ship.getVelocity().y, 5+50*effectLevel, 0, 0, 2*effectLevel, false, false, false);
                }  
            }
