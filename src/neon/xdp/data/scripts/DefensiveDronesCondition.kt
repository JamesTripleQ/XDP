...
class DefensiveDronesCondition : BaseMarketConditionPlugin() {
        ...
        try {
            if (market.primaryEntity != null) {
                ...
                if (!entity.hasScriptOfClass(DefensiveDronesFleetMananger::class.java)) {
                    entity.addScript(DefensiveDronesFleetMananger(entity, 1000f, 90f, 2, 9))
                }
            }
        } catch (e: Throwable) {}

        var industry = market?.industries?.find { it.spec.hasTag("xdp_centralprocessing") } ?: return
    ...
    override fun createTooltipAfterDescription(tooltip: TooltipMakerAPI?, expanded: Boolean) {
      /*  var color = market.textColorForFactionOrPlanet
        ...
        tooltip.addPara("This market hosts several autonomous launch platforms for defensive drones. Despite their age, they are still functional, even if severely weaker than in their prime.\n\n" +
                "Automatically launches up to 9 derelict drone fleets to protect the planet and system. The strength of the fleets depends on the planets fleet size multiplier." +
                "", 0f, Misc.getTextColor(), Misc.getHighlightColor(), "9", "derelict", "fleet size")
        tooltip.addSpacer(10f)
