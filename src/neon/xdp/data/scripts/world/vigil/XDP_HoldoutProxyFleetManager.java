...
public class XDP_HoldoutProxyFleetManager implements RouteManager.RouteFleetSpawner, EveryFrameScript, FleetEventListener {
	...
	public XDP_HoldoutProxyFleetManager(MarketAPI source) {
		this.market = source;
	}
	@Override
	...
	public void advance(float amount) {
		...
		if (tracker.intervalElapsed()) {
			// TODO: probably remove this on a save-breaking update
	...
	public String getRouteSourceId() {
		return "xdp_" + market.getId() + "_" + "derelicts";
	}
	...
	public static CampaignFleetAPI createPatrol(FleetFactory.PatrolType type, String factionId, RouteManager.RouteData route, MarketAPI market, Vector2f locInHyper, Random random) {
		...
		fleet.getMemoryWithoutUpdate().set("$xdp_holdoutProxy", true);
		if (!Global.getSector().getMemoryWithoutUpdate().contains("$xdp_HHtOffDone")) {
			fleet.getMemoryWithoutUpdate().set(MemFlags.WILL_HASSLE_PLAYER, true);
			fleet.getMemoryWithoutUpdate().set(MemFlags.HASSLE_TYPE, "xdp_HHtOff");
		}
	...
	public static void addProxyInteractionConfig(CampaignFleetAPI fleet) {
		fleet.getMemoryWithoutUpdate().set(MemFlags.FLEET_INTERACTION_DIALOG_CONFIG_OVERRIDE_GEN,
				new XDP_ProxyFleetInteractionConfigGen());
	}

	public static class XDP_ProxyFleetInteractionConfigGen implements FleetInteractionDialogPluginImpl.FIDConfigGen {
