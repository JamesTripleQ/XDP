package neon.xdp.data.scripts.campaign.econ.industries;
...
import neon.xdp.data.scripts.campaign.graid.XDP_DKBPrintRaidObjectivePlugin;
...
public class XDP_CentralProcessing extends BaseIndustry implements MarketImmigrationModifier, GroundRaidObjectivesListener {
	...
	public void modifyRaidObjectives(MarketAPI market, SectorEntityToken entity, List<GroundRaidObjectivePlugin> objectives, MarketCMD.RaidType type, int marineTokens, int priority) {
		...
		for (GroundRaidObjectivePlugin objective : list) {
			if (objective instanceof BlueprintGroundRaidObjectivePluginImpl) {
				...
				objectives.add(new XDP_DKBPrintRaidObjectivePlugin(market));
			}
		}
	}
