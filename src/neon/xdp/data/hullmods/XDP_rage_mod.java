...
import neon.xdp.data.scripts.util.XDP_superderelict_util;

import java.awt.*;
import java.util.HashSet;
import java.util.List;

public class XDP_rage_mod extends BaseHullMod {
	...
	// SOUND
	public static final String SOUND_ID = "xdp_rampage_start";
	public static final String SOUND_ID2 = "system_burn_drive_deactivate";
		...
		if (data.triggered) {
			...
			if (sparkleInterval.intervalElapsed()) {
				for (int x = 0; x < 3; x++) {
					...
					if (ship != null) {
						//color fuckery
						Color color1 = XDP_superderelict_util.randomiseColor(new Color(255, 100, 150, 255), 0, 100, 50, 0, true);
						Color color3 = XDP_superderelict_util.randomiseColor(new Color(255, 100, 150, 155), 0, 100, 50, 0, true);
			...
			if (data.glitchCooldown <= 0f) {
				...
				if (shouldActivate) {
					...
					engine.addFloatingText(loc, "EMERGENCY REPAIR", 32.0f, color3, ship, 0.5f, 1.0f);
